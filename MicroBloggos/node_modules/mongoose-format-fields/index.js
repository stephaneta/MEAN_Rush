...
var format_fields_plugin = function (schema, plugin_options) {
    ...
    var manageObject = function (obj, tags, prefix, return_always, parent_tags) {
        var output = {};
        for (var name in obj) {
            if (obj.hasOwnProperty(name)) {
                ...
                var value = getValueByType(field_name, obj[name], type, tags, parent_tags);
                if (value !== undefined) {
    ...
    var getFieldTags = function (field_name, parent_tags) {
        return (schema.tags_schema[field_name]) ? schema.tags_schema[field_name] : parent_tags;
    }

    var getValueByType = function (field_name, value, type, tags, parent_tags) {
        var requested_tags = getFieldTags(field_name, parent_tags);
        switch (type) {
            case 'array': {
                var array = [],
                ...
                for (var i = 0; i < value.length; i++) {
                    ...
                    if (e.toString && e.toString() != '[object Object]') {
                        ...
                        array.push(manageObject(e, tags, field_name+'.$.', true, requested_tags));
                    }
                }
            ...
            case 'object': {
                return manageObject(value, tags, field_name+'.', false, requested_tags);
            }
            default: {
                if (requested_tags && isAllowed(requested_tags, tags)) {
                    return value;
    ...
    schema.getFieldGrants = function (field_name) {
        return schema.getFieldTags(field_name);
    };
